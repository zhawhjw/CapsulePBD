...
let gui, previousAction='Random', activeAction;

let plane, dist, angle, sceneEntities = [];

const api = { state: 'Noise', camera: 'Camera' };
let activeCamera;
...
function generateTrees(expr) {
    let treeCtr = 0, totalTreeNum=500, zPos, xPos, spacing = 22;
    treePositions = [];
    switch (expr) {
    case 'Random':
        ...
        {        
            ...
            treePositions.push( {"x": Math.random()*world.x -world.x/2, "y":0, "z": Math.random()*world.z -world.z/2});
            treeCtr+=1;
        }
        break;
    case 'Blue Noise':
        ...
        {        
            treePositions.push( {"x": pnts[treeCtr][0] - world.x/2, "y":0, "z": pnts[treeCtr][1]- world.z/2 });                
            treeCtr+=1;
        }
        while(treeCtr<totalTreeNum)
        {
    ...
    case 'Rule-Based':
       while(treeCtr<totalTreeNum)
        {        
            zPos = -world.z/2 + Math.floor(treeCtr/spacing)  *   world.z / spacing;
            xPos = -world.x/2 + (treeCtr%spacing) *   world.x / spacing;
            treePositions.push( {"x": xPos, "y":0, "z": zPos });                
            treeCtr+=1;
        }
        break;
    default:
...
function createGUI( ) {
    const states = ['Rule-Based', 'Random', 'Blue Noise' ]; // TODO append your noise name to this list 
...
function animate() {
    ...
    for (var i = 0; i < sceneEntities.length; i++) {
        ...
        fpsCamera.position.set(dist * Math.cos(angle) , 
                               sceneEntities[i].position.y+7, 
                               dist * Math.sin(angle));
        
        fpsCamera.lookAt( dist * Math.cos(0.5*Math.PI + angle + 0.05), 
                          sceneEntities[i].position.y-13, 
                          dist * Math.sin(0.5*Math.PI  + angle+0.05));
...
function blueNoise(world) {
    ...
    var ratio = world.x/world.z, //window.devicePixelRatio || 1,
        width = world.x,
        height = world.z,
        radius = 1.5 * ratio,
        x0 = radius,
...
function possionDiscSamples(width,height,r,k)
{
    let grid = [];
    let tau =2 * Math.PI;
    let cellsize = r / Math.sqrt(2);
    let grid_width = Math.ceil(width / cellsize);
    let grid_height = Math.ceil(height / cellsize);

    function grid_coords(p)
    {
        return {"x": Math.floor(p.x / cellsize), "y":Math.floor(p.y / cellsize)}
    }

    function createRange(min_,max_) {
        let curr = min_, result = [];
        while(curr<max_)
        {
            result.push(curr);
            curr+=1;
    ...
    {
        let yrange = createRange(Math.max(gy-2,0), Math.min(gy+3,grid_height));
        let xrange =  createRange(Math.max(gx - 2, 0), Math.min(gx + 3, grid_width));
        let xi=0,yi,x,y,g;
        while(xi<xrange.length)
        {
            x= xrange[xi];
            yi=0;            
            while(yi<yrange.length)
            {
                y= yrange[yi];
    ...
    let i=0, gridSize = grid_width * grid_height;
     while(i<gridSize)
    {
        grid.push( undefined );
        i+=1;
    ...
    let pnt = new THREE.Vector2(width * Math.random(), height * Math.random())
    let queue = [pnt]
    let grid_pos = grid_coords(pnt)
    grid[grid_pos.x + grid_pos.y * grid_width] = pnt

    let alpha,d,px,py,qi,qx,qy,ctr=0;
    while(queue.length>0)
    {
        qi = Math.floor(Math.random() * queue.length)
        qx = queue[qi].x
        qy = queue[qi].y
        //queue[qi] = queue[-1] // [pop last element]
        //queue.pop()
        queue[qi] = queue.pop()// [pop last element]
        ctr=0;
        while(ctr<k)
        {
            alpha = tau * Math.random()
            d = r * Math.sqrt(3 * Math.random() + 1)
            px = qx + d * Math.cos(alpha)
            py = qy + d * Math.sin(alpha)
            //if(!(0 <= px < width && 0 <= py < height))
            if(!( (0 <= px) && (px < width) && (0 <= py) && (py< height)))
            {
                ctr+=1;
                continue;
            }        
            pnt = new THREE.Vector2(px, py)
            grid_pos = grid_coords(pnt)
            if(!fits(pnt, grid_pos.x, grid_pos.y))
            {
                ctr+=1;
                continue;
            }        
